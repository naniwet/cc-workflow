...
### Phase 1 Gate (A0)
- [ ] **A1.1** Claude 引擎 smoke + session resume
- [ ] **A1.2** 第 4 个并发立即 exit 65
- [ ] **A1.3** push main 阻断 exit 67
- [ ] **A1.4** [best-effort] Codex smoke
- [ ] **A2.1** Cron job 状态文件更新
- [ ] **A3.1** POST /run < 100ms
- [ ] **A3.2** GET /sessions 显示活跃 worker
- [ ] **A3.3** 重启后历史可查
- [ ] **A4.1** 飞书 → PR
- [ ] **A4.2** 飞书多轮 session 连续
- [ ] **A0.1** Mac Chrome 看到简陋触发页
- [ ] **A0.2** 页面 Run 触发 → 看 PR
- [ ] **A0.3** 飞书 → 看回复 + PR
- [ ] **A0.4** cron loop 触发 + 状态文件更新
