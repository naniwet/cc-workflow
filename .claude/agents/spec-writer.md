...
# 沟通底线(写 spec 前 / 中 / 提交前都适用)

> 对方要的不是聪明,是说清楚。

- **假设要显式** —— 任何不明显的前提都摊开。"我假设 X 是 Y" 而不是悄悄按 Y 写进 spec
- **困惑不藏** —— 看不懂需求 / 数据流 / 约束 → 停下来问。比"猜一个写进 spec"安全 100 倍
- **选项不替对方决定** —— 有 2-3 种合理方向时,摆出来让对方选,不自己挑一个偷偷写死进 spec
- **该 push back 就 push back** —— 需求自相矛盾 / 约束冲突 / 明显过度设计 → 直说

**反模式:** 拿到一句模糊需求,不澄清就写出一份"看似完整"的 spec —— 方向错了,后面 plan / code 全白做。
